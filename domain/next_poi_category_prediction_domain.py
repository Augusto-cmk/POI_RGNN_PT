...
#            minimum = 300
        ...
        max_userid = len(df)
        print("Quantidade de usuários: ", max_userid)
        # update users id
        df['id'] = np.array([i for i in range(max_userid)])
    ...
    def run_tests_one_location_output_k_fold(self,
        ...
        for i in range(k_folds):
            ...
            for j in range(n_replications):
                ...
                model = self._find_model(dataset_name, model_name)(step_size=sequences_size,
                                                           location_input_dim=number_of_categories,
                                                           num_users=num_users,
                                                           time_input_dim=48).to(device=DEVICE)
    ...
    def _train_and_evaluate_model(self,
        ...
        x_train_tensors = numpy_to_torch(*X_train)
        x_test_tensors = numpy_to_torch(*X_test)

        opt = optim.Adam(model.parameters(), lr=0.001, betas=(0.8, 0.9))
        Loss = nn.CrossEntropyLoss()

        # Prepare DataLoader
        train_dataset = TensorDataset(*x_train_tensors, torch.tensor(y_train, dtype=torch.int64).to(device=DEVICE))
        test_dataset = TensorDataset(*x_test_tensors, torch.tensor(y_test, dtype=torch.int64).to(device=DEVICE))
        ...
        for epoch in range(epochs):
            ...
            for batch in train_loader:
                inputs, targets = batch[:-1], batch[-1].float()
                opt.zero_grad()
                [print(inp.size()) for inp in inputs]
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                outputs = model(*inputs)
                loss = Loss(outputs,targets)
                y_predict_location.extend(outputs.cpu().numpy())
...
def numpy_to_torch(*args):
    DEVICE = torch.device("mps" if torch.backends.mps.is_available() else "cuda" if torch.cuda.is_available() else "cpu")
    return [torch.tensor(arg, dtype=torch.int32).to(device=DEVICE) for arg in args]